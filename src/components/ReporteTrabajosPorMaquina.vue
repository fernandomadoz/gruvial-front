<template>
    ...
    <v-card>
        <v-card-title>Trabajos por Máquinas</v-card-title>
        <v-card-text>
            <!-- BOTONES -->
            <v-row no-gutters>
...
const traerValoresDeReportes = async () => {
    ...
    let bodyMes = await axios.post(ENDPOINT_PATH_API.value + "traer-valores-reporte-trabajos-por-maquina-por-mes", jsonCliente, { headers: headersAxios.value[0] });
    let bodyAnio = await axios.post(ENDPOINT_PATH_API.value + "traer-valores-reporte-trabajos-por-maquina-por-anio", jsonCliente, { headers: headersAxios.value[0] });
...
const chartData = computed(() => {
    ...
    data.labels = trabajosFiltradas.map(trabajo => trabajo.nombre_de_maquina);

    let chartTrabajos = { data: null, label: null, backgroundColor: null }
    chartTrabajos.data = trabajosFiltradas.map(trabajo => Number(trabajo.importe));
    chartTrabajos.label = 'Gastos';

<template>
    ...
    <v-card>
        <v-card-title>Cotizaciones por Estado</v-card-title>
        <v-card-text>
            <!-- BOTONES -->
            <v-row no-gutters>
...
const cotizaciones = ref(null)
const cotizacionesMes = ref(null)
const cotizacionesAnio = ref(null)
const mes = ref(1)
const anio = ref(2024)
...
chartOptions.value = {
    ...
    plugins: {
        ...
        title: {
            display: true,
            text: 'Cotizaciones por Categoria'
        },
    }
}
...
const traerValoresDeReportes = async () => {
    ...
    let bodyMes = await axios.post(ENDPOINT_PATH_API.value + "traer-valores-reporte-cotizaciones-por-estado-por-mes", jsonCliente, { headers: headersAxios.value[0] });
    let bodyAnio = await axios.post(ENDPOINT_PATH_API.value + "traer-valores-reporte-cotizaciones-por-estado-por-anio", jsonCliente, { headers: headersAxios.value[0] });
    //console.log(body['data'])

    cotizacionesMes.value = bodyMes['data']['cotizaciones']
    cotizacionesAnio.value = bodyAnio['data']['cotizaciones']
...
const chartData = computed(() => {
    ...
    if (mes.value === 0) {
        cotizaciones.value = cotizacionesAnio.value
        cotizacionesFiltradas = cotizaciones.value.filter(cotizacion => cotizacion.firma_id === firma_id.value && cotizacion.anio == anio.value);
    }
    else {
        cotizaciones.value = cotizacionesMes.value
        cotizacionesFiltradas = cotizaciones.value.filter(cotizacion => cotizacion.firma_id === firma_id.value && cotizacion.anio == anio.value && cotizacion.mes === mes.value);
    }



    data.labels = cotizacionesFiltradas.map(cotizacion => cotizacion.estado);

    let chartCotizaciones = { data: null, label: null, backgroundColor: null }
    chartCotizaciones.data = cotizacionesFiltradas.map(cotizacion => Number(cotizacion.cantidad));
    chartCotizaciones.label = 'Cotizaciones';
    chartCotizaciones.backgroundColor = [
    ...
    data.datasets.push(chartCotizaciones)
    loading.value = false

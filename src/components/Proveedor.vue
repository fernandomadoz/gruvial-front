<template>
    ...
    <v-card>
      ...
      >
        <v-tab value="Encabezado">Proveedor</v-tab>
      </v-tabs>
...
<script setup> 
  ...
  const proveedor_id = ref(Number(props.proveedor_id))
console.log(proveedor_id.value)
  ...
  if (props.accion == 'A') {
    ...
    const body_find = await axios.get(ENDPOINT_PATH_API.value + "proveedor/"+proveedor_id.value, {headers: headersAxios.value[0]})
    proveedor = body_find['data']
    titulo = 'Proveedor ID: '+proveedor_id.value
    nombre_o_razon_social.value = proveedor.data.nombre_o_razon_social
    cuit_o_cuil.value = proveedor.data.cuit_o_cuil
    telefonos.value = proveedor.data.telefonos
    direccion.value = proveedor.data.direccion
    condicion_iva_id.value = proveedor.data.condicion_iva_id
    email.value = proveedor.data.email
  ...
  */
  ...
  const tipo_de_proveedor_idRules =  [
    v => !!v || 'Es requerido',
  ];
  ...
  async function enviarFormEncabazado() {
    ...
    let tipo_de_proveedor_id_value = tipo_de_proveedor_id.value
    if (typeof(tipo_de_proveedor_id_value) == 'object') {
      tipo_de_proveedor_id_value = tipo_de_proveedor_id_value.id
    }
    ...
    */
    ...
    //Si proveedor_id = -1 creo el trabajo sino actualizo
    if (props.accion == 'A') {
      const body_new = await axios.post(ENDPOINT_PATH_API.value + "proveedor", json, {headers: headersAxios.value[0]})
      cod_mensaje = 'MA'
      ...
      console.log(body_new)
      console.log(body_new['data'].hasError)
    ...
    loading.value = false
    if (!hasError.value) {
      router.push('/proveedores-list/'+cod_mensaje);
    }
  }

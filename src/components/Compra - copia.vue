    ...
    <div class="d-flex my-2" v-if="props.compra_encabezado_id!=-1">
      <v-row>
        <v-col cols="12" sm="6" md="4">
          <v-chip class="w-100" size="x-large" label prepend-icon="mdi-hammer-wrench" v-if="!es_mensual">Importe: <span class="text-blue pl-3">${{ Number(importe_de_compra).toLocaleString("es-AR", 'ARS') }}</span></v-chip>
        </v-col>
        <v-col cols="12" sm="6" md="4">
          <v-chip class="w-100" size="x-large" label prepend-icon="mdi-text-box-outline">Facturado: <span class="text-blue pl-3">${{ Number(importeFacturas).toLocaleString("es-AR", 'ARS') }}</span></v-chip>
        </v-col>
        <v-col cols="12" sm="6" md="4">
          <v-chip class="w-100" size="x-large" label prepend-icon="mdi-currency-usd">Pagado: <span class="text-blue pl-3">${{ Number(importePagos).toLocaleString("es-AR", 'ARS') }}</span></v-chip>
        </v-col>
      </v-row>
    </div>
            ...
            <v-row>                
                ...
                <v-col cols="12" sm="6" md="4" v-show="!es_mensual">
                    ...
                    <MoneyField
                        ...
                        label="Importe Total de Compra (MoneyField)"
                        required="required"
                        :rules="importeRules"
              ...
              <!-- INICIO PAGO -->
                ...
                <!-- INICIO CHEQUE -->
                  <v-row v-show="(tipo_de_cobro_id == 3 || tipo_de_cobro_id == 4) && carga_pago">
                    ...
                    <v-col cols="12" sm="6" md="4" v-show="cheque_en_cartera">
                        <v-autocomplete
                            v-model="cheque_id"
...
<script setup> 
  ...
  const firmasDelaCompra = ref(null)
